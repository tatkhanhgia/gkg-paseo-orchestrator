...
- **Open source:** This distribution is AGPL-3.0-or-later licensed. You can inspect it, modify it, and build on it.
...
This distribution adds role- and skill-bound Foundation workflows on top of the upstream product.
...
- **Portfolios:** Portfolios group related Projects on one Host so you can manage work that spans multiple codebases. Archiving a Portfolio leaves its Projects and their running work active. A Project belongs to at most one active Portfolio.

Current downstream state and candidate additions:

- Desktop (Electron), mobile (iOS/Android), web, CLI
- Current downstream provider support: Claude Code, Codex, Cursor, Antigravity, Pi, and Codex-derived custom providers. Other upstream adapters remain source-only and disabled.
