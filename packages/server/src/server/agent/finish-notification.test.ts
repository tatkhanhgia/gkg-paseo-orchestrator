...
test("a durable-record write failure preserves the observed run across restart before a newer run", async () => {
  ...
  // The record write failed, so no delivery/prompt should exist yet, and
  // the watch must still be active with the exact observed run receipt.
  await vi.waitFor(async () => {
    const child = await h.agentStorage.get("child-1");
    expect(child?.finishNotificationWatches).toEqual([
      expect.objectContaining({
        watchId: watch.watchId,
        status: "active",
        observedRunId: "turn-1",
  ...
  // Restart with the same provider run receipt. Resume must recover the same
  // watch identity; a later launch must mint a new watch rather than reusing
  // the old observed-run identity.
  const h2 = createFinishHarness();
  for (const [id, record] of h.records) h2.records.set(id, record);
  h2.agents.set("child-1", makeFakeAgent("child-1", "running"));
  h2.agents.get("child-1")!.activeForegroundTurnId = "turn-1";
  h2.agents.set("caller-1", makeFakeAgent("caller-1", "idle"));
