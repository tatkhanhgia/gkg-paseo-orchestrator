...
function toLaunchRoleBinding(
  roleBinding: NonNullable<PersistedLaunchContract["roleBinding"]>,
): NonNullable<AgentLaunchContext["roleBinding"]> {
  ...
  return {
    ...
    ...(roleBinding.executionProfile
      ...
      : {}),
    ...(roleBinding.roleProfile ? { allowedSkills: roleBinding.roleProfile.allowedSkills } : {}),
...
export class AgentManager {
  ...
  private async createAgentInternal(
    ...
    // Role assertions run on the plugin-transformed config so a hook cannot bypass them.
  ...
  private async reloadAgentSessionInternal(
    ...
    this.assertReloadPreservesLaunchContract(existing, overrides);
    ...
    try {
      ...
      // The previous agent is already closed, so the tool catalog needs the binding until the
      // replacement session registers.
      ...
      if (rehydrateFromDisk) {
        await this.resetTimelineForRehydrate(agentId);
      }

      // Preserve existing labels and timeline during reload.
      handedToRegistration = true;
      const registration = this.registerSession(session, storedConfig, agentId, {
      ...
      void registration
        .finally(() => this.roleBindingsAwaitingRegistration.delete(agentId))
        .catch(() => undefined);
      return registration;
    } catch (error) {
      if (closedExisting) {
  ...
  private assertReloadPreservesLaunchContract(
  ...
  private async resetTimelineForRehydrate(agentId: string): Promise<void> {
  ...
  private async buildLaunchContext(
    ...
    const context: AgentLaunchContext = {
      ...
      ...(roleBinding ? { roleBinding: toLaunchRoleBinding(roleBinding) } : {}),
