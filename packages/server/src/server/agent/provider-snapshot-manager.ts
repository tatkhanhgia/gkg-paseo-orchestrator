...
export class ProviderSnapshotManager {
  ...
  private createAgentManagerState(
    ...
    overrides: Record<string, ProviderOverride> | undefined = this.providerOverrides,
  ...
  private getRoleBindingSupport(
    ...
    const client = definition ? this.ensureClient(provider, definition, clients) : null;
  ...
  private createGeneration(
    definitions: Record<AgentProvider, ProviderDefinition>,
    overrides: Record<string, ProviderOverride> | undefined,
    clients: Record<AgentProvider, AgentClient> = this.providerClients,
  ): RegistryGeneration {
