...
async function markWatchStopped(
  ...
  await updateRecord(childAgentId, (record) => {
    ...
    return { record: { ...record, finishNotificationWatches: watches }, result: undefined };
  });
}
 ...
 * only after this watch has a durable started-run receipt; an idle snapshot
 * before that point remains UNKNOWN. Callers that can control launch ordering
 * attach before dispatch, while legacy dispatch-first callers still benefit
 * from the same receipt check. This closes the fast-finish race without
 * synthesizing completion after a prelaunch crash.
 */
export function attachFinishNotificationWatch(
  ...
  let hasSeenRunning = Boolean(observedRunId && observedRunStartedAt);
  ...
  function stop(): void {
    ...
    stopped = true;
    unsubscribe();
    void markWatchStopped(dependencies, childAgentId, watch.watchId);
  }
  ...
  function detectSafely(
    ...
    void detect(reason, options.permissionRequest)
      .then(async () => {
        if (terminal) {
  ...
  // to dispatch) a run before calling this, so an already-terminal snapshot
  // here is a genuine outcome, not "hasn't started yet".
  ...
  if (childSnapshot.lifecycle === "running") {
    ...
    // fast completed run. Only the durable observed run receipt authorizes a
    // finish notification.
...
export async function resumePendingFinishNotificationDeliveries(
  ...
  for (const record of allRecords) {
    ...
    for (const watch of record.finishNotificationWatches ?? []) {
      ...
      const hasPendingTerminalDelivery = (callerRecord?.finishNotificationDeliveries ?? []).some(
        (delivery) =>
      ...
      if (hasPendingTerminalDelivery) continue;
      stopWatches.push(
