  ...
  return {
    ...
    mutationBoundary:
      effectClass === "mutating"
        ...
        : { mode: "no-write" },
    externalEffectBoundary: assignmentExternalEffectBoundaryFor("lead", effectClass),
...
test("orders a concurrent replacement after a pending accepted steer", async () => {
  ...
  try {
    ...
    expect(session.interruptCount).toBe(1);
    // The replacement prompt is recorded in the continuation that accepts its provider turn,
    // which can land a microtask after startTurn observes the second start.
    await vi.waitFor(() =>
      expect(
        manager
...
test("role-bound create persists immutable binding and passes only launch instructions", async () => {
  ...
  try {
    ...
    expect(client.launchContexts[1]?.roleBinding).toEqual({
      ...
      noWrite: true,
      mandatoryResourceReads: created.roleBinding?.harnessBinding?.resources.map((resource) => ({
        key: resource.key,
        path: resource.path,
...
test("ordinary non-SLP agents remain available when the bundled SLP artifact is unavailable", async () => {
  ...
  try {
    ...
    // With the SLP artifact rejected there is no active role-policy generation, so role
    // creation fails closed at trusted policy selection.
    expect(() => manager.preflightRoleCreate({ provider: "codex", roleId: "lead" })).toThrow(
      /trusted_policy_active_selection_ambiguous: .*found 0/,
    );
  } finally {
    rmSync(workdir, { recursive: true, force: true });
  }
