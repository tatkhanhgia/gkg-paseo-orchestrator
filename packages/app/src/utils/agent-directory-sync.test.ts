...
function replaceFetchedAgentDirectory(input: { serverId: string; entries: FetchAgentsEntry[] }) {
  return new AgentStoreProjection(input.serverId).replaceFetched(input.entries);
}

function applyAgentDirectoryDelta(input: {
  serverId: string;
  delta: Parameters<AgentStoreProjection["applyDelta"]>[0];
}) {
  return new AgentStoreProjection(input.serverId).applyDelta(input.delta);
}
