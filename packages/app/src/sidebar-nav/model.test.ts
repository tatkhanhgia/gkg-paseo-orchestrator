...
// Paseo Foundation coordination builtins follow the upstream builtins.
const COORDINATION_KEYS = ["topology", "portfolios", "rooms", "councils"];
...
describe("resolveSidebarNavItems", () => {
  it("yields builtins then plugins, all visible, when nothing is stored", () => {
    ...
    expect(summarize(items)).toEqual([
      ...
      ...COORDINATION_KEYS.map((key) => ({ key, visible: true })),
      { key: kanbanKey, visible: true },
      { key: notesKey, visible: true },
    ]);
    expect(items[8]).toEqual({ kind: "plugin", key: kanbanKey, group: kanban, visible: true });
...
describe("moveSidebarNavItem", () => {
  ...
  it("moves an item down", () => {
    const next = moveSidebarNavItem({ items, key: "councils", direction: "down", previous: [] });
...
describe("builtinSidebarNavShortcutAction", () => {
  it("maps only the builtins that have a keyboard shortcut", () => {
    ...
    for (const key of COORDINATION_KEYS) {
      expect(builtinSidebarNavShortcutAction(key as never)).toBeNull();
    }
  });
});
